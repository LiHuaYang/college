	...
	$.getJSON("./WGetCleanerWorkArrangesByPageServlet",{page:page},function(json){
		for ( var i = 0; i < json.length; i++) {
			...
			for(var j = 0;j < $(".mrtruediv").length; j++){
				$(".mrtruediv").eq(j).find(".thisgroup").append('<tr onclick=do_onclick(this)   onselectstart="return false;"'+co+'">'+
						'<td>'+json[i].cleanerWorkId+' </th>'+
						'<td>'+json[i].cleanerId+'</th>'+
						'<td>'+json[i].roomId+' </th>'+
						'<td>'+cleanerWorkState+'</th>'+
				'</tr>')
			}
		}
...
function toconsume(page) {
	...
	"<div id=addg onclick='changebut(this),toconsume(1)'>消费记录</div><div style='width:24%;' id=adde onclick='changebut(this),toworkarrange(1)'>清洁员工作安排记录</div></div>")	
	...
	$.getJSON("./WGetConsumptionRecordPageServlet",{page:page},function(json){
		for ( var i = 0; i < json.length; i++) {
			...
			for(var j = 0;j < $(".mrtruediv").length; j++){
				$(".mrtruediv").eq(j).find(".thisgroup").append('<tr onclick=do_onclick(this) ondblclick=do_blcclick(this) onmousemove="changeTrColorone(this)" onmouseout="changeTrColortwo(this)"   onselectstart="return false;" style="overflow: hidden-x;height:60px;font-size:12px;font-weight: normal;-moz-user-select:none;background-color: '+co+'">'+
...
function torecharge(page) {
	$(".mrtruediv").html("");
	$(".mrtruediv").append("<div id=addgroup><div style='width:24%;' id=adde onclick='changebut(this),toloadpeople(1)'>清洁记录</div><div style='width:24%;' id=addg onclick='changebut(this),torecharge(1)'>充值记录</div>"+
	"<div id=adde onclick='changebut(this),toconsume(1)'>消费记录</div><div style='width:24%;' id=adde onclick='changebut(this),toworkarrange(1)'>清洁员工作安排记录</div></div>")	
	$(".mrtruediv").append('<div id=mccontent> '+
			'<table>'+
			'<tbody>'+
			'<tr>'+
