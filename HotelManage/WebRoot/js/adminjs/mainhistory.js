...
function toworkarrange(page) {
	$(".mhtruediv").html("");
	$(".mhtruediv").append("<div id=addgroup><div style='width:24%;' id=adde onclick='changebut(this),toloadpeople(1)'>清洁记录</div><div style='width:24%;' id=adde onclick='changebut(this),torecharge(1)'>充值记录</div>"+
	"<div style='width:24%;' id=adde onclick='changebut(this),toconsume(1)'>消费记录</div><div style='width:24%;' id=addg onclick='changebut(this),toworkarrange(1)'>清洁员工作安排记录</div></div>")	
	...
	$.getJSON("./WGetCleanerWorkArrangesByPageServlet",{page:page},function(json){
		for ( var i = 0; i < json.length; i++) {
			...
			for(var j = 0;j < $(".mrtruediv").length; j++){
				$(".mhtruediv").eq(j).find(".thisgroup").append('<tr onclick=do_onclick(this)    onselectstart="return false;" style="overflow: hidden-x;height:60px;font-size:12px;font-weight: normal;-moz-user-select:none;background-color: '+co+'">'+
						'<th style="font-weight: normal;">'+json[i].cleanerWorkId+' </th>'+
						'<th style="font-weight: normal;">'+json[i].cleanerId+'</th>'+
						'<th style="font-weight: normal;">'+json[i].roomId+' </th>'+
						'<th style="font-weigh	t: normal;">'+cleanerWorkState+'</th>'+
				'</tr>')
			}
		}
...
function toconsume(page) {
	$(".mhtruediv").html("");
	$(".mhtruediv").append("<div id=addgroup><div style='width:24%;' id=adde onclick='changebut(this),toloadpeople(1)'>清洁记录</div><div style='width:24%;' id=adde onclick='changebut(this),torecharge(1)'>充值记录</div>"+
	"<div style='width:24%;' id=addg onclick='changebut(this),toconsume(1)'>消费记录</div><div style='width:24%;' id=adde onclick='changebut(this),toworkarrange(1)'>清洁员工作安排记录</div></div>")	
	$(".mhtruediv").append('<div id=mccontent> '+
			'<table  style="font-size:13px;"   width="100%" >'+
			'<tbody>'+
			'<tr style="height:50px;background-color:rgb(180,180,180);font-size:13px;">'+
		...
		for ( var i = 0; i < json.length; i++) {
			...
			for(var j = 0;j < $(".mrtruediv").length; j++){
				$(".mhtruediv").eq(j).find(".thisgroup").append('<tr onclick=do_onclick(this) ondblclick=do_blcclick(this) onmousemove="changeTrColorone(this)" onmouseout="changeTrColortwo(this)"   onselectstart="return false;" style="overflow: hidden-x;height:60px;font-size:12px;font-weight: normal;-moz-user-select:none;background-color: '+co+'">'+
						'<th style="font-weight: normal;">'+json[i].consumptionId+' </th>'+
						'<th style="font-weight: normal;">'+json[i].consumptionTime+'</th>'+
						'<th style="font-weight: normal;">'+json[i].consumptionMoney+' </th>'+
						'<th style="font-weigh	t: normal;">'+json[i].consumptionName+'</th>'+
						'<th style="font-weight: normal;">'+json[i].consumptionPhon+'</th>'+
						'<th style="font-weight: normal	;">'+json[i].consumptionIDNumber+'</th>'+
						'<th style="font-weight: normal	;">'+json[i].consumptionAddress+'</th>'+
						'<th style="font-weight: normal	;">'+json[i].userid+'</th>'+
				'</tr>')
			}
		}
...
function torecharge(page) {
	...
	"<div style='width:24%;' id=adde onclick='changebut(this),toconsume(1)'>消费记录</div><div style='width:24%;' id=adde onclick='changebut(this),toworkarrange(1)'>清洁员工作安排记录</div></div>")	
	$(".mhtruediv").append('<div id=mccontent> '+
			'<table  style="font-size:13px;"   width="100%" >'+
			'<tbody>'+
			'<tr style="height:50px;background-color:rgb(180,180,180);font-size:13px;">'+
	...
	$.getJSON("./WGetRechageRecordPageServlet",{page:page},function(json){
		for ( var i = 0; i < json.length; i++) {
			...
			for(var j = 0;j < $(".mrtruediv").length; j++){
				$(".mhtruediv").eq(j).find(".thisgroup").append('<tr onclick=do_onclick(this) ondblclick=do_blcclick(this) onmousemove="changeTrColorone(this)" onmouseout="changeTrColortwo(this)"   onselectstart="return false;" style="overflow: hidden-x;height:60px;font-size:12px;font-weight: normal;-moz-user-select:none;background-color: '+co+'">'+
						'<th style="font-weight: normal;">'+json[i].rechargeId+'</th>'+
						'<th style="font-weight: normal;">'+json[i].rechargeTime+'</th>'+
						'<th style="font-weight: normal;">'+json[i].rechargeMoney+' </th>'+
						'<th style="font-weigh	t: normal;">'+json[i].userId+'</th>'+
				'</tr>')
			}
		}
