...
public class WChangeRoomDetailServlet extends HttpServlet {
...
//		int	RoomLongitude =Integer.parseInt(request.getParameter("RoomLongitude"));//	房间经度
		...
		int RoomEquipmentId = Integer.parseInt(request.getParameter("RoomEquipmentId"));//房间设备号
		...
		//房间地址
		int AddressId = Integer.parseInt(request.getParameter("AddressId"));//房间地址Id
		...
		RoomEquipment roomEquipment = new RoomEquipment(Breakfast, AirConditioner, CoatHanger, DryingMachine, Heating, HotTub, IndoorFirePlace, Iron, Kitchen, Necessities, Network, PETFacilities, SwimmingPool, TV, WashingMachine, WheelchairAccessible, WorkingSpace,RoomEquipmentId);
		Room room = new Room(RoomNumber, RoomUnitPrice, RoomType, RoomUsingState, RoomLockState, RoomLongitude, RoomLatitude, RoomMassage, RoomPassword, RoomEquipmentId, RoomPicture, AddressId, RoomDetailAddress,RoomId);
		Address address = new Address(AddressProvince, AddressCity, AddressVillage, RoomId, AddressId);
		
		boolean bisupdate = ibBllFrame.UptateRoom(room, roomEquipment, address);
